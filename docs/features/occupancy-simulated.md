...
2. Dans l'écran consommateur (`MapScreen.tsx`), démarrer un timer qui appelle `updateOccupancy([{ planId, color }])` avec une couleur qui change à chaque tick :
   ```ts
...
4. Exposer un contrôle utilisateur pour démarrer/arrêter la simulation (ici, un bouton toggle réutilisant le champ "Place ID" déjà présent pour `goToPlace`) — une feature du catalogue doit être démontrable via une interaction, pas seulement câblée en silence.

## Points d'attention

- **`planId` doit être un vrai ID de POI de la carte chargée.** `venue.pois.find((p) => p.id === entry.planId)` échoue silencieusement (pas d'erreur remontée) si l'ID ne correspond à rien — vérifier avec le même champ que celui qui fonctionne déjà pour "Go" (`select_place`).
